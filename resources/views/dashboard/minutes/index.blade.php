<x-app-layout>
    ...
    {{-- Judul Halaman --}}
    <x-slot name="title">Data Notulen</x-slot>
        ...
        <div class="bg-gray-50 rounded-lg border border-gray-300 shadow">
            <div class="p-2 space-y-2">
                <div class="flex flex-col lg:flex-row items-center justify-between gap-4">
                    {{-- Tombol Tambah --}}
                    <x-buttons.primary-button href="{{ route('dashboard.minute.create') }}"
                        class="w-full lg:w-auto text-center bg-green-600 hover:bg-green-700">
                        Tambah
                    </x-buttons.primary-button>

                    {{-- Form Filter & Search --}}
                    <form method="GET" action="{{ route('dashboard.minute.index') }}"
                        ...
                        <div class="w-full lg:w-80">
                            <x-forms.input type="text" name="search"
                                placeholder="Cari masalah atau ditindaklanjuti oleh..." autocomplete="off"
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-800 text-gray-50">
                    <tr>
                        ...
                        <th class="p-2 font-normal text-left border-r border-gray-600">Masalah</th>
                        <th class="p-2 font-normal text-left border-r border-gray-600">Ditindaklanjuti Oleh</th>
                        <th class="p-2 font-normal text-center border-r border-gray-600">Batas Tindaklanjut</th>
                ...
                <tbody class="divide-y divide-gray-300">
                    @forelse($minutes as $minute)
                        <tr class="hover:bg-blue-50">
                            <td class="p-2 text-center border-r border-gray-300">
                                {{ ($minutes->currentPage() - 1) * $minutes->perPage() + $loop->iteration }}</td>
                            <td class="p-2 border-r border-gray-300 whitespace-nowrap">
                                {{ Str::limit(strip_tags($minute->problem), 25) }}
                            </td>
                            <td class="p-2 border-r border-gray-300 whitespace-nowrap">
                                {{ Str::limit($minute->user->name, 20) }}</td>
                            ...
                            <td class="p-2 text-center border-r border-gray-300 whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($minute->created_at)->format('d/m/Y H:i') }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="p-4 text-center text-gray-500">Tidak ada data notulen</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
